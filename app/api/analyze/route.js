export async function POST(request) {
  ...
  "Функции, которые уже реально подключены сейчас: анализ ТЗ через OpenAI; локальная обработка изображений через Sharp — изменение размера, центрирование, белый фон, JPG/PNG/WebP; удаление фона через Replicate. " +
  "Улучшение качества изображений, сложное AI-редактирование, карточки товара как полный автоматический процесс, сайты, дизайн и видео пока считаются запланированными, пока для них не подключены реальные обработчики. "+
