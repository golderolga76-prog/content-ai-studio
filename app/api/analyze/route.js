...
NEEDS_INPUT
если для выполнения не хватает файла, текста, ��азмера,
...
- Если клиент просит визуально наклонить, повернуть, изменить угол или положение объекта, используй replicate:flux-edit, когда это нельзя выполнить обычным Sharp без изменения композиции или искажения результата.
- Если клиент просит изменить композицию, взаимное расположение объектов или ракурс, используй replicate:flux-edit.
- Если клиент просит аккуратно отредактировать товар с сохранением формы, упаковки, логотипов, надписей и внешнего вида, используй replicate:flux-edit.
- Формулировки «слегка наклонить баночку вправо/влево», «повернуть товар», «изменить положение товара», «сохранить надписи и внешний вид» являются явным запросом на визуальное AI-редактирование, а не на ручную операцию и не на неподключённый handler.
- Для такого шага handler должен быть ровно "replicate:flux-edit", status — "AVAILABLE", cost — "paid", requiresConfirmation — true.
- В params.prompt передавай конкретную инструкцию на русском языке, составленную из запроса клиента, без добавления неподтверждённых деталей. Например: "Слегка наклонить баночку вправо, сохранить упаковку, надписи, форму и внешний вид без изменений".
